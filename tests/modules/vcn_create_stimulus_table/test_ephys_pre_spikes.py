...
def test_assign_sweep_values():
    ...
    pd.testing.assert_frame_equal(
        expected, obtained, check_like=True,
...
@pytest.mark.parametrize(
    ...
    [
        [
            ...
            True,
            pd.DataFrame({
                "Pos_x": [0, 1], "Pos_y": [1, 2], "count": [12, 42]
            }),
        ],
        [
...
def test_apply_display_sequence(sweeps, disp_seq, expected):
    ...
    pd.testing.assert_frame_equal(
        obt_table, expected_table, check_like=True,
        check_column_type=False, check_dtype=False
    )
...
def test_make_spontaneous_activity_tables(stimulus_tables, expected):
    ...
    obtained = ephys_pre_spikes.make_spontaneous_activity_tables(
        stimulus_tables
    )

    if len(obtained) == 1:
        pd.testing.assert_frame_equal(
            obtained[0], expected[0], check_like=True,
            check_column_type=False, check_dtype=False
        )
    else:
...
def test_create_stim_table(
    ...
    pd.testing.assert_frame_equal(
        obtained, expected, check_like=True,
...
def test_build_stimuluswise_table(stimulus, stf, start_key, end_key, expected):
    ...
    for obtained_table, expected_table in zip(obtained, expected):
        pd.testing.assert_frame_equal(
            obtained_table, expected_table, check_like=True,
            check_column_type=False, check_dtype=False
        )

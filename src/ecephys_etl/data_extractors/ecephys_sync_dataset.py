...
class EcephysSyncDataset(Dataset):
    ...
    def __init__(self):
        """In-memory representation of a sync h5 file as produced by
        the sync package.

        Notes
        -----
        base is from here:
        http://aibspi/mpe_apps/sync/blob/master/sync/dataset.py
        ...
        object. To make a new SyncDataset in client code, use the factory
        classmethod. This is done for ease of testability.
        """
        pass

    def extract_led_times(
        self,
        ...
        try:
            ...
            warnings.warn(
                f"Unable to find LED times using line labels {keys}, "
                f"returning line {fallback_line}")
    ...
    def extract_frame_times_from_photodiode(
        ...
        irregular_interval_policy = functools.partial(
            stimulus_sync.allocate_by_vsync, np.diff(vsync_times)
        )
        frame_indices, frame_start_times, frame_end_times = (
    ...
    def factory(cls, path):
        ...
        ----------
        path : str
            Filesystem path to the h5 file containing sync information to
            be loaded.
        """
        obj = cls()
        obj.load(path)

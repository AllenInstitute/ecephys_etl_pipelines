...
from argschema.fields import (
    Nested, String, Float, Dict, List, Bool
)
...
class InputParameters(ArgSchema):
    ...
    minimum_spontaneous_activity_duration = Float(
        ...
        help=(
            "Detected spontaneous activity sweeps will be rejected if "
    ...
    frame_time_strategy = String(
        ...
        help=(
            "Technique used to align frame times. Options are "
            "'use_photodiode', which interpolates frame times between "
            "photodiode edge times (preferred when vsync times are "
            "unreliable) and 'use_vsyncs', which is preferred when reliable "
            "vsync times are available."
        )
    )
    ...
    fail_on_negative_duration = Bool(
        default=False,
        help=(
            "Determine if the module should fail if a "
